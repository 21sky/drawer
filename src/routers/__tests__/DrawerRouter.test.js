...
it('handles basic drawer logic and fires close on switch', () => {
    const ScreenA = () => React.createElement("div", null);
    const ScreenB = () => React.createElement("div", null);
...
it('handles initial route navigation', () => {
    const FooScreen = () => React.createElement("div", null);
    const BarScreen = () => React.createElement("div", null);
    const router = DrawerRouter({
        Foo: {
            screen: FooScreen,
        },
...
it('drawer opens, closes and toggles', () => {
    ...
    const state2 = router.getStateForAction({ type: DrawerActions.OPEN_DRAWER }, state);
    expect(state2.isDrawerOpen).toEqual(true);
    const state3 = router.getStateForAction({ type: DrawerActions.CLOSE_DRAWER }, state2);
    expect(state3.isDrawerOpen).toEqual(false);
    const state4 = router.getStateForAction({ type: DrawerActions.TOGGLE_DRAWER }, state3);
...
it('drawer opens, closes with key targeted', () => {
    ...
    const state = router.getStateForAction(INIT_ACTION);
    const state2 = router.getStateForAction({ type: DrawerActions.OPEN_DRAWER, key: 'wrong' }, state);
    expect(state2.isDrawerOpen).toEqual(false);
    const state3 = router.getStateForAction({ type: DrawerActions.OPEN_DRAWER, key: state.key }, state2);
...
it('drawerRouter will close drawer on child navigaton, not on child param changes', () => {
    ...
    BarView.router = BarRouter;
    const router = DrawerRouter({
        Bar: BarView,
        Foo: FooView,
    });
    const emptyState = router.getStateForAction(NavigationActions.init());
    const initState = router.getStateForAction(DrawerActions.openDrawer(), emptyState);
    expect(initState.isDrawerOpen).toBe(true);
    const state0 = router.getStateForAction(NavigationActions.navigate({ routeName: 'Quo' }), initState);
    ...
    const state1 = router.getStateForAction(NavigationActions.setParams({
...
it('goBack closes drawer when inside of stack', () => {
    const ScreenA = () => React.createElement("div", null);
    const DrawerScreen = () => React.createElement("div", null);
    ...
    const state1 = router.getStateForAction(NavigationActions.navigate({ routeName: 'Foo' }), state0);
    expect(state1.index).toEqual(1);
    const state2 = router.getStateForAction(DrawerActions.openDrawer(), state1);
    const state3 = router.getStateForAction({ type: DrawerActions.DRAWER_OPENED }, state2);
